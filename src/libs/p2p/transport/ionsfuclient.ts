import { TypeUtil, UUID } from '../../util/util'
...
export class IonSfuSignal implements Signal {
	...
	async call<T>(params: any): Promise<T> {
		const id = UUID.string(null, null)
		params.id = id
...
export class IonSfuClient {
	...
	init(targetPeerId: string, router: any, conf: Configuration) {
		...
		if (!conf.iceServers) {
			conf.iceServers = config.appParams.iceServer[0]
		}
		...
		if (this._client.transports[0]) {
			let transport = this._client.transports[0]
			...
			this._pc.onicecandidateerror = (ev) => {
				console.info('onicecandidateerror:' + ev.errorText)
			}
	...
	ondatachannel() {
		let _that = this
		this._client.ondatachannel = function (event) {
...
export class IonSfuClientPool {
	...
	private _ionSignalAction: IonSignalAction = null
	...
	registSignalAction(ionSignalAction: IonSignalAction) {
	...
	create(peerId: string, router: any): IonSfuClient {
		...
		client = new IonSfuClient(peerId, router, null)
	...
	async receiveData(event: any) {
		...
		let {
			receiveHandler
		} = ionSfuClientPool.getProtocolHandler(config.p2pParams.chainProtocolId)
		if (receiveHandler) {

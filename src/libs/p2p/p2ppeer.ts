...
import { myself, Myself, myselfPeerService } from './dht/myselfpeer'
import { peerProfileService } from './dht/peerprofile'
import { openpgp } from './crypto/openpgp'
import { dispatchDatastore, dispatchPool } from './datastore/ds'
...
export class P2pPeer {
  ...
  public chainProtocolId: string = config.p2pParams.chainProtocolId
  ...
  async initMyself(password: string, myselfPeer: any): Promise<Myself> {
    ...
    */
    ...
    myselfPeer.publicKey = await openpgp.export(keyPair.publicKey, null)
  ...
  */
  ...
  registEvent() {
    this.host.on('peer:discovery', (peerId) => {
   ...
   */
  async dialProtocol(peer: any, protocols: string[], options: any): Promise<{ stream:*, protocol:string }> {

...
const SliceLimit = 100 * 1024 * 1024
...
export class DataBlockService extends BaseService {
	...
	static async verify(dataBlock: DataBlock) {
		...
		if (peerId === myself.myselfPeer.peerId) {
	...
	static async merge(dataBlocks: DataBlock[]): Promise<DataBlock> {
		if (dataBlocks && dataBlocks.length > 0) {
			...
			let slicePayloads = await Promise.all(ps)
	...
	async blockMapMerge(targetMap, dbMap) {
		if (dbMap) {
			let blocks = []
			// 每个不同的块号循环
			for (let key in dbMap) {
	 ...
	 * @param {*} options
	 */
	async _find(connectPeerId, conditionBean = {}, options = { createPeer: false, receiverPeer: true }) {
		let blockMap
	...
	async loadUrl(url, uriVariables) {
		let html
	 ...
	 * @param {*} blockId
	 */
	async findTx(connectPeerId, blockId) {
	 ...
	 * @param {*} blockId
	 */
	async findTxPayload(connectPeerId, blockId) {
		let start = new Date().getTime()
